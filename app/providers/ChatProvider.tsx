...
export const ChatProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    const fetchChannels = async () => {
      try {
        const response = await fetch('https://backend.readtalk.workers.dev/channels', {
          headers: {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        // Fetch all users
        const usersResponse = await fetch('https://backend.readtalk.workers.dev/users', {
          headers: {
            'X-Session-Id': localStorage.getItem('session') || '',
          }
        });
        ...
        // Fetch online users
        const onlineResponse = await fetch('https://backend.readtalk.workers.dev/users/online', {
          headers: {
            'X-Session-Id': localStorage.getItem('session') || '',
          }
        });
  ...
  const loadChannelMessages = async (channelId: string, before?: string) => {
    try {
      ...
      const url = new URL(`https://backend.readtalk.workers.dev/channels/${channelId}/messages`);
      if (before) {
