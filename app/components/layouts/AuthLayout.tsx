...
export default function AuthLayout() {
    ...
    return (
        <div className="flex flex-col h-screen">
            <div className="flex w-full justify-between border-b border-neutral-200 bg-neutral-50 py-1 pr-2 pl-4 sm:pl-6 transition-colors dark:border-neutral-800 dark:bg-neutral-950">
            ...
            <div className="flex flex-1 overflow-hidden">
                {/* Sidebar */}
                <div className="w-full sm:w-56 px-2 sm:px-3 pt-4 sm:pt-6 border-r border-neutral-200 transition-colors dark:border-neutral-800 overflow-y-auto">
                    {/* Channels Section */}
                    <div className="px-1 sm:px-2 mb-2 flex justify-between items-center">
                        <h2 className="text-[10px] sm:text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase">
                            Channels
                        </h2>
                        <Plus 
                            size={16} 
                    ...
                    {isLoadingChannels ? (
                        <div className="px-1 sm:px-2 py-1.5 text-xs sm:text-sm text-neutral-500">Loading channels...</div>
                    ) : (
                        channels.map((channel) => (
                            <Link
                                key={channel.id}
                                to={`/channel/${channel.id}`}
                                className="flex items-center px-1 sm:px-2 py-1 text-xs sm:text-sm text-neutral-700 dark:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-900 rounded cursor-pointer"
                            >
                    ...
                    {/* Users Section */}
                    <div className="px-1 sm:px-2 mt-4 sm:mt-6 mb-2 flex justify-between items-center">
                        <h2 className="text-[10px] sm:text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase">
                            Users
                        </h2>
                        <Plus 
                    ...
                    {onlineUsers.map((user) => (
                        <div 
                            ...
                            className="flex items-center px-1 sm:px-2 py-1 text-xs sm:text-sm text-neutral-700 dark:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-900 rounded cursor-pointer"
                        >
                            <span className="mr-1 sm:mr-2 size-1.5 sm:size-2 rounded-full bg-green-500"></span>
                    ...
                    {offlineUsers.map((user) => (
                        <div 
                            key={user.id}
                            className="flex items-center px-1 sm:px-2 py-1 text-xs sm:text-sm text-neutral-700 dark:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-900 rounded cursor-pointer opacity-60"
                        >
                            <span className="mr-1 sm:mr-2 size-1.5 sm:size-2 rounded-full bg-neutral-400"></span>
                            <span className="truncate">{user.first_name} {user.last_name}</span>
                        </div>
                    ))}
                </div>
